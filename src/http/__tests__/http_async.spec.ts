...
test('should emit error when CookieJar#getCookies throws error.', async () => {
  ...
  jest.spyOn(jar, 'getCookies').mockImplementation((...args: unknown[]): void | Promise<unknown> => {
    const cb = args.at(-1);
...
test('should emit error when CookieJar#setCookie throws error.', async () => {
  ...
  jest.spyOn(jar, 'setCookie').mockImplementation((...args: unknown[]): void | Promise<unknown> => {
    const cb = args.at(-1);
    if (typeof cb === 'function') {
      cb(new Error('Error'));
    } else {
